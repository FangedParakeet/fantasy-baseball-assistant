...
class TeamPitchingRotations(DB_Recorder):
    ...
    def initialise_team_rotations(self):
        ...
        self.team_rotations = self.get_team_rotations_from_recent_games(merged_games)
        self.initialise_projected_pitcher_count()
        logger.info(f"Built rotations for {len(self.team_rotations)} teams")
    ...
    def initialise_projected_pitcher_count(self):
        ...
        for team in teams_with_rotations:
            ...
            if team_confirmed_games:
                ...
                last_confirmed_pitcher_id = team_confirmed_games[0]['pitcher_id']
                
                # Find this pitcher's position in the rotation
                try:
                    last_pitcher_index = rotation.index(last_confirmed_pitcher_id)
                    ...
                    logger.warning(f"Last confirmed pitcher {last_confirmed_pitcher_id} for team {team} not found in rotation {rotation}")
    ...
    def get_probable_pitchers_after_date(self, start_date):
        records = self.get_records_with_conditions(self.probable_pitchers_table, [start_date], ['team', 'player_id AS pitcher_id', 'game_date'], ['game_date >= %s', "accuracy = 'confirmed'"], ['game_date DESC'])
        return records
    ...
    def get_team_rotations_from_recent_games(self, recent_games):
        team_rotations = defaultdict(list)
        ...
        for game in recent_games:
            ...
            if pitcher_id in seen_pitchers[team]:
                # We've completed the rotation when we see a duplicate
                logger.info(f"Team {team} rotation complete: {team_rotations[team]} ({len(team_rotations[team])} pitchers)")
                completed_teams.add(team)
                continue
            
            # Add pitcher to rotation (insert at beginning to reverse chronological order)
            team_rotations[team].insert(0, pitcher_id)
            seen_pitchers[team].add(pitcher_id)
        
        # Log teams that didn't complete their rotation
        for team in team_rotations:
            if team not in completed_teams:
                logger.warning(f"Team {team} rotation incomplete: {team_rotations[team]} ({len(team_rotations[team])} pitchers) - may need more recent games")
        
        logger.info(f"Built rotations for {len(team_rotations)} teams")
        return team_rotations
        ...
        try:
            ...
            query = f"""
                WITH team_recent_games AS (
                    ...
                    FROM (
                        SELECT home_team as team, home_pitcher_id as pitcher_id, game_date
                        ...
                        SELECT away_team as team, away_pitcher_id as pitcher_id, game_date
                        FROM {self.game_pitchers_table} gp
    ...
    def merge_game_lists(self, primary_list: list[dict], secondary_list: list[dict]) -> list[dict]:
        ...
        Duplicates are identified by (team, pitcher_id, game_date) tuple.
        """
        seen = set()
        merged = []
        ...
        for game in primary_list:
            key = (game['team'], game['pitcher_id'], game['game_date'])
            if key not in seen:
                seen.add(key)
                merged.append(game)
