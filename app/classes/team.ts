...
class Team {
	...
	async updateAllOpponentLeagueTeams(): Promise<void> {
		let [teams] = await this.db.query<Team[]>(`SELECT ${this.TEAM_SELECT_COLUMNS.join(', ')} FROM ${this.teamsTable} WHERE is_user_team = false`);
		if (teams.length === 0) {
	...
	async getAllPlayersWithPosition( position: string ): Promise<Player[]> {
		...
		const [players] = await this.db.query<Player[]>(
            `SELECT name, mlb_team FROM ${this.playersTable} WHERE ${flagKey} = ? AND status = ?`,
            [1, 'rostered']
		);
		return players;
	}
	...
	async getMyLeagueKey(): Promise<string | null> {
		...
		if (!userResult) {
			throw new Error('Failed to get user leagues');
		}
